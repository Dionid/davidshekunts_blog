...
Основная фишка данной технологии Static Site Generator (SSG) –предварительная копиляция сайта.

Компиляция позволяет взять шаблоны интерфейса (начиная со стандартных систем типа Pug, EJS, заканчивая
React.js, Vue.js и так далее), взять данные с сервера и создать из них готовые статические html страницы.

Пример: у вас блог на 100 постов. К вам зашел на сайт клиент:

WordPress: *при каждом запросе* берет шаблон, берет данные из БД, объеденяет их и отдает клиенту.

SSG: у вас уже заранее приготовленная (скопилированная) html страница с контентом, которую вы сразу же отдаете клиенту.

**Когда именно происходит компиляция?**

Вы можете запускать ее вручную или настроить (через готовые инструменты) автоматическиую компиляцию файлов при любых
...
"Headless" – означает "не имеющий интерфейса" и это правда так. Если в WordPress у вас есть в Теме сразу идет и административная,
и клиентская панель, то Headless предполагает максимум наличие административной панели, а вот клиентскую вы делаете сами.
